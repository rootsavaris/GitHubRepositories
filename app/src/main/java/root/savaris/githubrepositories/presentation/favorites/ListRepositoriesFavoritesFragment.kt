...
class ListRepositoriesFavoritesFragment : Fragment() {
            ...
            viewModel.navigateToRepository.observe(this, Observer { pairRepository ->
                pairRepository?.let {
                    findNavController().navigate(
                        ListRepositoriesFavoritesFragmentDirections.actionListRepositoriesFavoritesFragmentToDetailRepositoryFragment(
                            pairRepository.first,
                            pairRepository.second
                        )
                    )
    ...
    private fun loadViews() {
        ...
        with(binding.containerRecyclerView.commonRecyclerView){
            ...
            listRepositoriesFavoritesAdapter =
                ListRepositoriesFavoritesAdapter(
                    activity!!,
                    RepositoryClickListener { repositoryOwner, repositoryName ->
...
class ListRepositoriesFavoritesViewModelFactory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>) = ListRepositoriesFavoritesViewModel(
        application,
        repository
    ) as T
}