...
class ListRepositoriersAdapter(private val activity: Activity, private val clickListener: RepositoryClickListener) : ListAdapter<RepositoryDataItem, RecyclerView.ViewHolder>(diffCallback) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == DEFAULT_VIEW) {
            val binding: ViewDataBinding = DataBindingUtil.inflate(LayoutInflater.from(activity), R.layout.adapter_repository_line, parent, false)
            RepositoryViewHolder(binding as AdapterRepositoryLineBinding)
        } else {
            val binding: ViewDataBinding = DataBindingUtil.inflate(LayoutInflater.from(activity), R.layout.adapter_load_more, parent, false)
